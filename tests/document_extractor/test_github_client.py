...
async def test_github_client_initialization() -> None:
    ...
    with pytest.raises(DocumentExtractorException, match="GITHUB_TOKEN is required"):
        GitHubClient(
            concurrent_requests=2,
            user_agent="test-agent",
            github_api_version="2022-11-28",
...
async def test_request_401(github_client: GitHubClient, mocker: MockerFixture) -> None:
    ...
    with pytest.raises(
        DocumentExtractorException, match="GITHUB_TOKEN is not authorized"
    ):
        await github_client.request("GET", "https://api.github.com/test")
...
async def test_request_unexpected_error(
    ...
    with pytest.raises(DocumentExtractorException, match="Unexpected error"):
        await github_client.request("GET", "https://api.github.com/test")
