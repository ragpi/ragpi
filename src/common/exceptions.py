...
def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    def handle_extractor_type_error(error: dict[str, Any]) -> dict[str, Any]:
        """Handle specific extractor type validation errors"""
        if error["ctx"]["discriminator"] != "'type'":
            return error

        valid_types = ", ".join(ExtractorType._value2member_map_.keys())
    ...
    def process_error(error: dict[str, Any]) -> dict[str, Any]:
        ...
        if error["type"] in ("union_tag_not_found", "union_tag_invalid"):
            error = handle_extractor_type_error(error)
